...
public class ModifiableProjectAppModel(ICollection<ISharedEventStreamHandler<Cid, KuboNomadEventStream, KuboNomadEventStreamEntry>> listeningEventStreamHandlers)
    : ModifiableProjectNomadKuboEventStreamHandler(listeningEventStreamHandlers)
{
    /// <summary>
    ...
    public async IAsyncEnumerable<(ReadOnlyUserNomadKuboEventStreamHandler User, Role Role)> GetCollaboratorsAsync([EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        foreach (var collaborator in Inner.Collaborators)
        {
            ...
            await userAppModel.AdvanceEventStreamToAtLeastAsync(EventStreamPosition?.TimestampUtc ?? DateTime.UtcNow, ContentPointerToStreamEntryAsync, cancellationToken).ToListAsync(cancellationToken);

            yield return (userAppModel, collaborator.Role);
        }
    }
    ...
    /// </summary>
    /// <param name="cancellationToken">A token that can be used to cancel the ongoing operation.</param>
    public async Task<ReadOnlyPublisherNomadKuboEventStreamHandler> GetPublisherAsync(CancellationToken cancellationToken)
    {
        var existingKeysEnumerable = await Client.Key.ListAsync(cancellationToken);
        var existingKeys = existingKeysEnumerable.ToOrAsList();
        ...
        ReadOnlyPublisherNomadKuboEventStreamHandler appModel = existingKeys.FirstOrDefault(x => x.Id == ipnsId) is { } value
            // If current node has write permissions
            ? new ModifiablePublisherAppModel(ListeningEventStreamHandlers)
            {
                Client = Client,
                Id = ipnsId,
                Sources = Sources,
            ...
            :
            // If current node has no write permissions
            new ReadOnlyPublisherAppModel(ListeningEventStreamHandlers)
            {
                Client = Client,
                Id = ipnsId,
                Inner = result,
